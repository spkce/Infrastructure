...
{
	InfraTrace("delete thread internal:%p\n", pInternal);
	delete pInternal;
}
...
bool CThread::stop(bool isBlock)
{
	...
	{
		//使用条件变量，等待线程退出
		InfraTrace("wait proc exit:%p\n",m_pInternal);
		pthread_join(m_pInternal->handle, NULL);
		InfraTrace("proc exit:%p\n",m_pInternal);
	}
